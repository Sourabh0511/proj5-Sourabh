...
func main() {
	...
	for _, ad := range args {
		blockStoreAddrs = append(blockStoreAddrs, ad)
	...
	log.Fatal(startServer(addr, strings.ToLower(*service), blockStoreAddrs)) //has to be changed
}
...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType == "both" {
		bsrv := surfstore.NewBlockStore()
		msrv := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterBlockStoreServer(grpcServer, bsrv)
		surfstore.RegisterMetaStoreServer(grpcServer, msrv)
